...
import { ImportErrorModal } from '../../../../component/importError/ImportErrorModal'
    ...
    <div className={`card ${className}`}>
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1 text-danger' >Import Error : {data.length}</span>
        </h3>
          ...
          <div className='tab-pane fade show active' id='kt_table_widget_8_tab_1'>
            <div className='table-responsive'>
              <table className='table align-middle gs-0 gy-3'>
                ...
                <tbody>
                  {
                    data && data?.map((errorList) => (
                     
                      <tr key = {errorList.import_error_id}>
                        <td>
                          <div className='symbol symbol-50px me-2'>
                            <span className='symbol-label bg-light-danger'>
                              <MdErrorOutline size={25} className='svg-icon-2x text-danger'></MdErrorOutline>
                            </span>
                          </div>
                        </td>
                        <td>
                        ...
                        <td className='text-center'>
                          <span className='text-danger fw-bold d-block fs-7'>{errorList.timestamp}</span>

                        </td>
                        <td className='text-center'> 
                        <a onClick={(e)=> {ImportErrorClick(e,errorList.import_error_id)}}><ImportErrorModal></ImportErrorModal></a>     
