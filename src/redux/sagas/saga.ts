...
import {put, all, takeLatest, fork, call, StrictEffect } from "redux-saga/effects";
import {
  TEST_ASYNC_RES,
  TEST_ASYNC_REQ,
  STATE_SUCCESS_RES,
...
function DagUpdate(is_paused: any) {
  ...
  axios.patch(Base_URL + "patch", json, axiosConfig)
    .then(function (response) {
...
function GetStateSuccess() {
  const result = axios.get(Base_URL + "get?dags=dags/Dag_Creater/dagRuns?state=success", axiosConfig)
    .then(function (response) {
      console.log(response);
...
function GetStateFailed() {
  const result = axios.get(Base_URL + "get?dags=dags/Dag_Creater/dagRuns?state=failed", axiosConfig)
    .then(function (response) {
      return response.data.data.body.total_entries;
    })
    .catch(function (error) {
...
function DagRuns(data:any){
  ...
  axios.post(Base_URL + "post",{dags},axiosConfig)
  .then(function (response) {
...
function* testSaga(action: any): Generator<
  StrictEffect,
  any,
  any
> {
