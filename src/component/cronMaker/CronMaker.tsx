...
const CronMaker: React.FC<Props> = ({className}) => {
  ...
  const Hour = (data: string) => {
    customAxiosCron.post('hours', {hours: data}).then(function (response) {
      setCronValue(response.data.data)
      dispatch(cronValue_REQ(response.data.data))
    })
  }
  ...
  const DailyDay = (hour: string, min: string) => {
    customAxiosCron.post('dailyDay', {hours: hour, minutes: min}).then(function (response) {
      setCronValue(response.data.data)
      dispatch(cronValue_REQ(response.data.data))
    })
  }
  const DailyWeek = (hour: string, min: string) => {
    customAxiosCron.post('dailyWeekday', {hours: hour, minutes: min}).then(function (response) {
      setCronValue(response.data.data)
      dispatch(cronValue_REQ(response.data.data))
    })
  }
  const Weekly = (hour: string, min: string, day: string) => {
    customAxiosCron.post('weekly', {hours: hour, minutes: min, day: day}).then(function (response) {
      setCronValue(response.data.data)
      dispatch(cronValue_REQ(response.data.data))
    })
  }
  const monthlyDay = (hour: string, min: string, day: string, month: string) => {
    customAxiosCron
      .post('monthlyDay', {hours: hour, minutes: min, day: day, month: month})
  ...
  const monthlyWeek = (min: string, hour: string, day: string, week: string, month: string) => {
    customAxiosCron
      .post('monthlyWeek', {hours: hour, minutes: min, day: day, week: week, month: month})
      .then(function (response) {
        setCronValue(response.data.data)
        dispatch(cronValue_REQ(response.data.data))
      })
  ...
  const yearlyMonthDay = (min: string, hour: string, day: string, month: string) => {
    customAxiosCron
      .post('yearlyMonthDay', {minutes: min, hours: hour, day: day, month: month})
      .then(function (response) {
        setCronValue(response.data.data)
        dispatch(cronValue_REQ(response.data.data))
      })
  }
  const yearlyMonthWeekDay = (
    min: string,
    hour: string,
    day: string,
    week: string,
    ...
    customAxiosCron
      .post('yearlyMonthWeekDay', {minutes: min, hours: hour, day: day, week: week, month: month})
      .then(function (response) {
        setCronValue(response.data.data)
        dispatch(cronValue_REQ(response.data.data))
      })
      ...
      <div className='card-body py-3'>
        <div className='tab-content'>
          ...
          <div className='tab-pane fade show active' id='kt_table_widget_5_tab_1'>
            ...
            <div className='form-control form-control-lg form-control-solid'>{cronValue}</div>
          ...
          <div className='tab-pane fade' id='kt_table_widget_5_tab_2'>
            ...
            <div className='row mb-12'>
              <span className='d-flex align-items-center me-2'>
                ...
                <span className='d-flex flex-column'>
                  <span className='fw-bolder text-gray-800 text-hover-primary fs-5'>Start at</span>
                </span>
              </span>
              ...
              <span className='d-flex align-items-center me-2'>
                <span className='form-check form-check-custom form-check-solid'>
                  <input
                    ...
                    value='yes'
                    name='daily'
                    checked={selected2 === 'yes'}
                    onChange={handleChange2}
                  />
                </span>
                <span className='d-flex flex-column'>
                  <span className='fw-bolder text-gray-800 text-hover-primary fs-5'>Everyday</span>
                </span>
              </span>
            </div>
            <div className='row mb-12'>
          ...
          <div className='tab-pane fade' id='kt_table_widget_5_tab_4'>
            ...
            <div
              className='btn btn-icon btn-primary w-90px h-40px pulse pulse-white'
              onClick={() => Weekly(weeklyHour, weeklyMin, checkedList.join(',').slice(1))}
            >
              Generate
            </div>
            <div className='form-control form-control-lg form-control-solid'>{cronValue}</div>
          ...
          <div className='tab-pane fade' id='kt_table_widget_5_tab_5'>
            <span className='d-flex align-items-center me-2'>
              <span className='form-check form-check-custom form-check-solid'>
                <input
                  ...
                  className='form-check-input'
                  name='monthly'
                  checked={selected4 === 'yes'}
                  onChange={handleChange4}
                />
              </span>
              <span className='d-flex flex-column'>
                <span className='fw-bolder text-gray-800 text-hover-primary fs-5'>Monthly Day</span>
              </span>
            </span>
            ...
            <div className='row mb-12'>
              <div className='col-md-6 fv-row'>
                <input
                  ...
                  className='form-control form-control-solid'
                  onChange={MonthlyDay}
                  value={monthlyDayvalue}
                />
              </div>
              <div className='col-md-6 fv-row'>
                <select
                  className='form-control form-control-solid'
                  onChange={MonthlyMonth}
                  value={monthlyMonthvalue}
                >
                  <option value=''>선택 해주세요</option>
                  <option value='1'>1</option>
                  <option value='2'>2</option>
            ...
            {selected4 === 'yes' ? (
              <div
                className='btn btn-icon btn-primary w-90px h-40px pulse pulse-white'
                onClick={() =>
                  monthlyDay(monthlyHour, monthlyMin, monthlyDayvalue, monthlyMonthvalue)
                }
              >
                Generate
              </div>
          ...
          <div className='tab-pane fade' id='kt_table_widget_5_tab_6'>
            ...
            {selected5 === 'yes' ? (
              ...
              <div
                className='btn btn-icon btn-primary w-90px h-40px pulse pulse-white'
                onClick={() =>
                  yearlyMonthWeekDay(yearlyMin, yearlyHour, yearlyDay2, yearlyWeek, yearlyMonth2)
                }
              >
                Generate
              </div>
