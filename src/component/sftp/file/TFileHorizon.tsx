...
import {createAccountSchemas, SftpInfo, SftpInitValues} from '../../../app/modules/wizards/components/CreateAccountWizardHelper'
...
const TFileHorizon: FC = () => {
  ...
  const connection_id_message = useSelector((state:RootState) => state.cronreducer.connection_id_message);
  const connection_name = useSelector((state:RootState) => state.cronreducer.connection_name);
  const connection_name_message = useSelector((state:RootState) => state.cronreducer.connection_name_message);
  
  const connection_type = useSelector((state:RootState) => state.connectionSFTPreducer.state.connection_type)
  ...
  const JpaSave = () =>{
    if(window.confirm("생성 하시겠습니까?")){
      customAxiosAirflow.post("post",{dags:"connections",connection_id:initValues.connection_id, conn_type : initValues.connection_type, host : initValues.host, login:initValues.login, password: initValues.password, port:initValues.port, extra: initValues.extra, description:initValues.description})
      .then(function () {
            ...
            {() => (
              <Form className='mx-auto mw-600px w-100 pt-15 pb-10' id='kt_create_account_form'>
                ...
                <div className='d-flex flex-stack pt-15'>
                  ...
                  <div>
                  {
                      (stepper.current?.currentStepIndex === 1 || stepper.current?.currentStepIndex === undefined) ? <button type='submit' className='btn btn-lg btn-success me-3' disabled={sftpConButton}>
                      ...
                      </button> : stepper.current?.currentStepIndex === 2 ? <button type='submit' disabled={checkDisabled} className='btn btn-lg btn-success me-3' >  <span className='indicator-label'>
                      Continue
                          <KTSVG
                            path='/media/icons/duotune/arrows/arr064.svg'
                            className='svg-icon-3 ms-2 me-0'
                      ...
                      </button>: <button type='submit' onClick={JpaSave} className='btn btn-lg btn-success me-3' >
                        <span className='indicator-label'>
