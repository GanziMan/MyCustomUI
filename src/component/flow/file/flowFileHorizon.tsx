...
import { createAccountSchemas, Flowfile, FlowfileInitValues } from '../../../app/modules/wizards/components/CreateAccountWizardHelper'
...
const FlowFileHorizon: FC = () => {
  ...
  const confFile = { params: {trans_type: initValues.trans_type, owner: initValues.owner, sensor_id : initValues.sensor_id, save_mode:initValues.save_mode, data_type : initValues.data_type, file_type_list: initValues.file_type_list, delimiter: initValues.delimiter, schedule_interval: initValues.schedule_interval, local_path: initValues.local_path, target_location:initValues.target_location+"/"+initValues.sensor_id, target_conn_id : initValues.target_conn_id, file_name : initValues.file_name, privacy_on_off:initValues.privacy_on_off} }
                  ...
                  <div>
                    {
                      ...
                      </button> : stepper.current?.currentStepIndex === 4 ? <button type='submit' disabled={sourceCheckDisabled} onClick={JpaSave} className='btn btn-lg btn-success me-3'>
                        <span className='indicator-label'>
                          {!isSubmitButton && 'Continue'}
                          {isSubmitButton && '생성'}
                          <KTSVG
