...
import { connectionDB_REQ, dagcreateDBMSTYPE_REQ, dagcreateTRANSTYPE_REQ, pathValue_REQ, sourceID_REQ } from '../../../redux/actions/creatorActions'
                                ...
                                <div className='d-flex text-center flex-column text-white pt-8'>
                                    <div className='row mb-12'>
                                        <div className='col-md-6 fv-row'>
                                            ...
                                            <div className='position-relative'>
                                                ...
                                                >
                                                    ...
                                                    {dbselect && dbselect?.map((flow: any) => {
                                                        if(flow.dbms_type !== null){
                                                            return (<option key={flow.id}>{flow.source_name}</option>);
                                                        }
                                        ...
                                        <div className='col-md-6 fv-row'>
                                            ...
                                            <div className='position-relative'>
                                                <input type="button" className='btn btn-lg btn-light-success' name='conButton' value={"커넥션 조회"}></input>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                {dbselect && dbselect?.map((flow: any) => {
                                                    {
                                                        ...
                                                        if (flow.source_name === dbConnection) {
                                                            ...
                                                            dispatch(dagcreateDBMSTYPE_REQ(flow.dbms_type));
                                                            return (<div key={flow.id}>{flow.trans_type}</div>)
                                                        }
                                                    }
                                                })}
                                            </div>
                                                    ...
                                                    {dbselect && dbselect?.map((flow: any) => {
                                                        {
                                                            ...
                                                            if (flow.source_name === dbConnection) {
                                                                ...
                                                                return (<div key={flow.id}>{flow.dbms_type}</div>)
                                                            }
                                                        }
                                                    })}
                                                </div>
                                                    ...
                                                    {dbselect && dbselect?.map((db: any) => {
                                                        {
                                                            if (db.connection_name === dbConnection) {
                                                                return (<div key={db.id}>{db.schema}</div>)
                                                            }
                                                        }
                                                    })}
                                                </div>
