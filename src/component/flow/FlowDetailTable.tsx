...
const FlowDetailTable: React.FC<Props> = ({ className, sensor_id }, { flowList }: FlowProps) => {
    ...
    useEffect(() => {
        const Runs = async () => {
            try {
                setLoading(true);
                await customAxiosAirflow.get(`get?dags=dags/${sensor_id}/dagRuns`)
    ...
    const DagRunDelete = (params :string,e :any)=> {
        customAxiosAirflow.delete(`delete?dags=dags/${sensor_id}/dagRuns/${params.replace("+","%2B")}`,
        )
        .then(function(response){
        }).then(function(error){
            console.log(error);
    ...
    flowList = data;
    if (loading) return (<Loader type="spin" color="RGB 값" message={"DAG 정보를 불러오는 중 입니다."} />)
    else return (
        <div className={`card ${className}`}>
            ...
            <div className='card-header border-0 pt-5'>
                ...
                <div className='card-toolbar'>
                    <a className='btn btn-sm btn-light-success'>
                        Run : {flowList.length}
                    </a>
                </div>
            </div>
                        ...
                        <tbody>
                            {
                                flowList && flowList.map((flow) => (
                                    <tr key={flow.dag_run_id}>
                                        <td>
                                            <div className='d-flex align-items-center' onClick={() => navigate('/tasklist', { state: { dag_run_id: flow.dag_run_id, sensor_id: sensor_id } })}>
                                        ...
                                        <td>
                                            {
                                                flow.state === "success" ? <span className='badge badge-light-success fs-7 fw-semibold'>{flow.state}</span> : flow.state === "failed" ? <><span className='badge badge-light-danger fs-7 fw-semibold'>{flow.state}</span></> : <span className='badge badge-light-warning fs-7 fw-semibold'>{flow.state}</span>
                                            }
                                        </td>
                                        <td className='text-end d-flex align-items-center'>
                                            <span className='badge badge-light-dark fs-7 fw-semibold' onClick={(e) =>{ DagRunDelete(flow.dag_run_id,e)}}>Delete</span>
                                        </td>
                                    </tr>
                                ))}
                        </tbody>
